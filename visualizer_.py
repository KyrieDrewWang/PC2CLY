...
from dataset.pc_utils import read_ply
import h5py
...
# point_cloud, _, extrusion_labels, _, _,\
# 		 _, _, _, _, _, _, _ = get_model(h5_file, mesh_info=True, operation=True)

def main(pc_path):
	_, MODEL_ID = os.path.split(pc_path)
	MODEL_ID = MODEL_ID.split('.')[0]
	point_cloud = read_ply(pc_path)
	# with h5py.File(extrusion_label_path, 'r') as fp:
	#     extrusion_labels = fp['sdf'][:]
	# extrusion_labels = extrusion_labels.astype(np.int32)
 
	extrusion_labels = np.zeros((point_cloud.shape[0], ))
	# extrusion_labels = (~extrusion_labels).astype(np.float64)
	...
	# extrusion_labels = extrusion_labels[idx[:NUM_POINTS]]
	...
	if USE_GT_3D:
		...
		with torch.no_grad():
			gt_extrusion_labels = torch.tensor(extrusion_labels[idx[:NUM_POINTS]], dtype=torch.long).unsqueeze(0).to(device)
...
if __name__ == "__main__":
	...
	source_txt = "/data/wc/extrude_net/data/goodcase.txt"
	base_dir = "/data/wc/Points2sketch/proj_log/pc2skh-50-test/output/47/in_pc"
	txt_lst = readIndex(source_txt)
	pbar = tqdm(txt_lst, total=len(txt_lst))
	for t in pbar:
		pc_path = os.path.join(base_dir, t + '.ply')
